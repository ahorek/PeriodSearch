...
void CalcStrategyAvx512::gauss_errc(struct globals& gl, const int n, std::vector<double>& b, int& error)
{
	...
	for (i = 1; i <= n; i++)
	{
		...
		if (l < n) {
            ...
            __m512d avx_a = _mm512_maskz_loadu_pd(mask, &a[icol][l]);
		    avx_a = _mm512_mask_mul_pd(avx_a, mask, avx_a, avx_pivinv);
		    _mm512_mask_storeu_pd(&a[icol][l], mask, avx_a);
		}
		...
		for (ll = 0; ll < n; ll++)
		{
			if (ll != icol)
			{
				...
				if (l < n) {
                	...
                	avx_a = _mm512_mask_fnmadd_pd(avx_a, mask, avx_aa, avx_dum);
