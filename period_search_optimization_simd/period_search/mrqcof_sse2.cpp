 ...
 * @date 8.11.2006
 */
void CalcStrategySse2::mrqcof(std::vector<std::vector<double>>& x1, std::vector<std::vector<double>>& x2, std::vector<double>& x3, std::vector<double>& y,
	...
	std::vector<double>& beta, int mfit, int lastone, int lastma, double& trial_chisq, globals& gl, const bool isCovar)
{
	...
	for (i = 1; i <= gl.Lcurves; i++)
	{
		...
		for (jp = 1; jp <= gl.Lpoints[i]; jp++)
		{
			...
			{
				CalcStrategySse2::conv(jp, ma, gl);
			}
