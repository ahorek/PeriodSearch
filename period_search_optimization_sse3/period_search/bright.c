...
double bright(double ee[], double ee0[], double t, double cg[], double dyda[], int ncoef)
{
	...
	//Dg_row = aligned_vector_m128d(MAX_N_FAC + 3);
	//dbr = aligned_vector_m128d(MAX_N_FAC + 3);
			...
			INNER_CALC_DSMU
				if (icmp & 2)
				{
					...
					//1
					   ...
					   //auto x = *(__m128d*)&Dg[i + 1];
					   //auto y = Dg_row[incl_count];
					   //y = x;
				...
				else
				{
					...
					avx_lmu0 = _mm_shuffle_pd(avx_lmu0, avx_11, 0); //abychom nedelili nulou (translated: so as not to divide by zero)
					//0
	...
	for (i = 0; i < cyklus1; i += 10) //5 * 2doubles
	{
		...
		//auto x = &(&Dg_row)[0][0]; // [dgi] ;
		//auto y = &x;
		//auto z = y[0][0];
		//            40               128                20
		//int xmax = ((cyklus1 / 10) * (incl_count / 4) * (dgi / 5));

		Dgrow = &Dg_row[0][dgi];
		//Dgrow = &Dg_row[0 + (dgi * cyklus1)];
		pdbr = dbr[0];
		Dgrow1 = &Dg_row[1][dgi];
		//Dgrow1 = &Dg_row[1 + (cyklus1 * dgi)];
		pdbr1 = dbr[1];
		Dgrow2 = &Dg_row[2][dgi];
		//Dgrow2 = &Dg_row[2 + (cyklus1 * dgi)];
		pdbr2 = dbr[2];
		Dgrow3 = &Dg_row[3][dgi];
		//Dgrow3 = &Dg_row[3 + (cyklus1 * dgi)];
		pdbr3 = dbr[3];
		...
		{
			Dgrow = &Dg_row[j][dgi];
			//Dgrow = &Dg_row[j + (cyklus1 * dgi)];
			pdbr = dbr[j];
			Dgrow1 = &Dg_row[j + 1][dgi];
			//Dgrow1 = &Dg_row[j + 1 + (cyklus1 * dgi)];
			pdbr1 = dbr[j + 1];
			Dgrow2 = &Dg_row[j + 2][dgi];
			//Dgrow2 = &Dg_row[j + 2 + (cyklus1 * dgi)];
			pdbr2 = dbr[j + 2];
			Dgrow3 = &Dg_row[j + 3][dgi];
			//Dgrow3 = &Dg_row[j + 3 + (cyklus1 + dgi)];
			pdbr3 = dbr[j + 3];
	...
	for (; i < ncoef03; i += 4) //2 * 2doubles
	{
		...
		int xmax = ncoef03 * incl_count * 2;

		Dgrow = &Dg_row[0][dgi];
		//Dgrow = &Dg_row[0 * xmax + dgi];
		pdbr = dbr[0];
		Dgrow1 = &Dg_row[1][dgi];
		//Dgrow1 = &Dg_row[1 * xmax + dgi];
		pdbr1 = dbr[1];
		Dgrow2 = &Dg_row[2][dgi];
		//Dgrow2 = &Dg_row[2 * xmax + dgi];
		pdbr2 = dbr[2];
		Dgrow3 = &Dg_row[3][dgi];
		//Dgrow3 = &Dg_row[3 * xmax + dgi];
		pdbr3 = dbr[3];
		...
		for (j = 4; j < incl_count; j += 4)
		{
			...
			Dgrow = &Dg_row[j][dgi];
			//Dgrow = &Dg_row[j * xmax + dgi];
			pdbr = dbr[j];
			Dgrow1 = &Dg_row[j + 1][dgi];
			//Dgrow1 = &Dg_row[(j + 1) * xmax + dgi];
			pdbr1 = dbr[j + 1];
			Dgrow2 = &Dg_row[j + 2][dgi];
			//Dgrow2 = &Dg_row[(j + 2) * xmax + dgi];
			pdbr2 = dbr[j + 2];
			Dgrow3 = &Dg_row[j + 3][dgi];
			//Dgrow3 = &Dg_row[(j + 3) * xmax + dgi];
			pdbr3 = dbr[j + 3];
