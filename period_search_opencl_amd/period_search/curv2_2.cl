...
void mrqcof_curve2(
	...
	double beta[], const int inrel, const int lpoints) //double* alpha,
{
	...
	int latmph, latmpl;
	latmph = CUDA_lastone / BLOCK_DIM;
	if (CUDA_lastone % BLOCK_DIM) latmph++;
	...
	const int inrel, const int lpoints)
{
	...
	mrqcof_curve2(CUDA_LCC, Fa, texsig, texWeight, texbrightness, (*CUDA_LCC).beta, inrel, lpoints); //(*CUDA_LCC).alpha,
}
